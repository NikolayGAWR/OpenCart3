...
//-----------------------------------------------------------------------------------------------------------------
        ...
        $arStr['SYMBOLS'] = preg_split('//u',$_POST['name'],-1,PREG_SPLIT_NO_EMPTY);;
        array_unshift($arStr['SYMBOLS'], "space");
        unset($arStr['SYMBOLS'][0]);
        $arMyString = [];

        foreach($arStr['SYMBOLS'] as $symbolKey => $symbol) {
            if($symbolKey % 2 !== 0 && $symbol != ' ' && $symbol != ',') {
                $arMyString[] = $symbol;
            }
            if($symbol == ' ' || $symbol == ',') {
                $arMyString[] = $symbol;
...
//-----------------------------------------------------------------------------------------------------------------
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$mail->setTo($this->config->get('config_email')); // кому
    ...
    public function upload() {
        ...
        if (!empty($this->request->files['file']['name'])) {
            ...
            for($i=0;$i<$countfiles;$i++){
                ...
                $filename = $this->request->files['file']['name'][$i];
                ...
                $content = file_get_contents($this->request->files['file']['tmp_name'][$i]);
                    ...
                    $json['file'][$i] = $file = $filename;

                    move_uploaded_file($this->request->files['file']['tmp_name'][$i], DIR_UPLOAD . $file);

                    // Скрыть имя загружаемого файла
